...
if($row['id'] != $_SESSION['id']){
    die('invaild credentials');
}

if(!$row['image_names'] == ""){
    $img_attachments = explode(":", $row['image_names']);
}
if(!$row['file_names'] == ""){
    $file_attachments = explode(":", $row['file_names']);
}

?>
<textarea id="contents_source" style="display:none;">
...
<script type="text/javascript">
 function loadContent() {
     ...
     attachments['image'] = [];
     <?php
     if(isset($img_attachments)){
	 foreach($img_attachments as $img){ ?>
     attachments['image'].push({
	 'attacher': 'image',
	 'data': {
	     'imageurl': '<?php echo($img); ?>',
	     'filename': '<?php echo(getbasename($img)); ?>',
	     'filesize': '<?php echo(filesize($img)); ?>',
	     'originalurl': '<?php echo($img); ?>',
	     'thumburl': '<?php echo($img); ?>'
	 }
     });
     <?php
     	 }
	 ...
	 'attacher': 'file',
	 'data': {
	     'attachurl': '<?php echo($file); ?>',
	     'filemime': '<?php echo(filetype($file)); ?>',
	     'filename': '<?php echo(getbasename($file)); ?>',
	     'filesize': '<?php echo(filesize($file)); ?>'
	 }
     });
	 <?php
	     }
...
<div id="site_content">
	...
	<!--
	   ...
	   -->
    <form name="tx_editor_form" id="tx_editor_form" action="modify_check.php" method="post" accept-charset="utf-8">
	<input type="hidden" name="no" value="<?php echo($_GET['no']); ?>">
	<div id="input-subject-box">
            제목 : <input type ="text" name = "subject" size ="90" value="<?php echo($row['title']); ?>">
	</div>
	    <!-- 에디터 컨테이너 시작 -->
